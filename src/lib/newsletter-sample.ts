/**
 * Shared sample data for newsletter preview pages.
 * Used by both the API route handler and the /newsletter/preview webpage.
 */

import type { NewsletterEmailProps } from '@/emails/NewsletterEmail';

const BASE =
  process.env.NEXT_PUBLIC_SITE_URL ??
  (process.env.VERCEL_URL ? `https://${process.env.VERCEL_URL}` : 'http://localhost:3000');

export const newsletterSampleData: NewsletterEmailProps = {
  issueNumber: '#001',
  issueDate: 'May 2026',
  editorialNote:
  ...
  posts: [
    {
      ...
      excerpt:
        "What if fenced code blocks could play music? We built an open-source toolkit that turns markdown into interactive instruments — drum pads, keyboards, sequencers — all running in the browser.",
      author: 'Ricardo',
      date: 'Mar 19 2026',
    ...
    },
    {
      ...
      excerpt:
        "A live session showing what Midicircuit can do in practice — Playgrounds, Circuits, and the whole workflow from scratch.",
  ...
  education: {
    ...
    exampleContent:
      "Quick test: load each sample, play it at velocity 30 and then 127. If there's no personality difference, your dynamics need work. The best packs feel alive at every velocity.",
